...
# Scatter Plot
Car_Data['isOutlierMahalanobisOutlier'] = (Car_Data.mahalanobisDistance > (Car_Data.mahalanobisDistance.mean() + 2*Car_Data.mahalanobisDistance.std())) | (Car_Data.mahalanobisDistance < (Car_Data.mahalanobisDistance.mean() - 2*Car_Data.mahalanobisDistance.std()))
# Plotting Price Outliers and Milage Outliers
plt.scatter(Car_Data.price[(Car_Data.isOutlierMahalanobisOutlier==True)], Car_Data.mileage[(Car_Data.isOutlierMahalanobisOutlier==True)], c='green', label='Outlier', s=100)
plt.scatter(Car_Data.price[(Car_Data.isOutlierMahalanobisOutlier==False)], Car_Data.mileage[(Car_Data.isOutlierMahalanobisOutlier==False)], c='blue', label='Non-Outlier', s=100)
plt.title('Value of Car vs. Mileage')
plt.xlabel("Value")
plt.ylabel("Mileage")
plt.legend(loc='upper right', shadow=True)
...
# Drop NA values temporarily for Plotting before deciding how to replace the NA's
...
Tuberculosis[['c_cdr_lo','e_inc_100k_hi','year','c_cdr','e_prev_100k_hi']].hist(xlabelsize=0.5)
# 4 - Apply a log transformation on the data. Numpy has a log function. and visualise. Observe the changes
Logged_Values = np.log(Tuberculosis[['c_cdr_lo','e_inc_100k_hi','year','c_cdr','e_prev_100k_hi']])
...
Normalised[col] = Normalised[col].map(lambda x: normalise_func(x,max_c,min_c))
## 6 - Now you can compare the means of each column.
Normalised.mean()
from pandas.tools.plotting import scatter_matrix
scatter_matrix(Normalised, alpha=0.2, figsize=(6, 6), diagonal='kde') # Correlation Matrix
