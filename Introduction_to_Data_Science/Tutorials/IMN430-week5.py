...
# Each Columns is represented a value in each vector - The vectors appear
# to be unsorted
...
London_Data = pd.read_csv(
    'http://staff.city.ac.uk/~sbbk529/Teaching/Resources/INM430/Week05/london-borough-profiles.csv',
    skiprows=1)
Numpy_Array = (
    London_Data[
        ['Two-year business survival rates 2012']]).as_matrix()
...
npos *= np.sqrt((Numpy_Array_Distances ** 2).sum()) / \
    np.sqrt((npos ** 2).sum())
