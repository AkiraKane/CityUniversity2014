...
# Plot only the data that shows female passengers aged 40 to 50 and who
# paid more than or equal to 40.

# 1
# Posible to Download directly from URL when using the read_csv function
Passenger_Data = pd.read_csv(
    "http://staff.city.ac.uk/~sbbk529/Teaching/Resources/INM430/passengerData.csv")
...
# 5
# Example I learnt from to filter on multiple columns df[(df['A'] > 1) |
# (df['B'] < -1)], dont forget the brackets...
Subset = Complete_Data_Set[
    (Complete_Data_Set['Sex'] == "female") & (
        Complete_Data_Set['Age'] >= 40) & (
            Complete_Data_Set['Age'] <= 50) & (
                Complete_Data_Set['Fare'] >= 40)]
