...
# Get a Overview of the Properties of the DataFrame - Check that numerics
...
# They are also values located in Devon - I have chosen to take the
# average of the 3 that are avaliable and replace the missing values when
# that value
All_Devon_Means = Master[Master['Area Name                                           ']
                         .apply(lambda x: 'Devon' in x)].mean()
NA_Fill = All_Devon_Means[Numerics_Master ==
                          'Indicator_value_Acute_sexually_transmitted_infections']
print ('NA value = %f' % (NA_Fill.values))
Master.Indicator_value_Acute_sexually_transmitted_infections = \
    Master.Indicator_value_Acute_sexually_transmitted_infections.fillna(
        NA_Fill.values)
...
# the mean of the four local boroughs, this will be used to replace the
# missing value
...
# Correlation between variables in each category on a Scatter matrix and
# Tablular view

# Domain - Our communities
Our_Communities = Master[['Indicator_value_Deprivation',
                          'Indicator_value_Proportion_of_children_in_poverty',
                          'Indicator_value_Statutory_homelessness',
                          'Indicator_value_GCSE_achieved_(5A-C_inc._Eng_&_Maths)',
                          'Indicator_value_Violent_crime',
                          'Indicator_value_Long_term_unemployment']]
...
# For the variables mentioned above they will be transformed using a log
# transformation
...
Young_Health = Master[
    [
        ...
        'Indicator_value_Smoking_in_pregnancy',
        'Indicator_value_Starting_breast_feeding',
        'Indicator_value_Alcohol-specific_hospital_stays_(under_18)',
...
# Perform the Kolmogorov-Smirnov test for goodness of fit of the Normal
# Distribution
...
Adult_Health = Master[['Indicator_value_Adults_smoking',
                       'Indicator_value_Increasing_and_higher_risk_drinking_',
                       'Indicator_value_Healthy_eating_adults',
                       'Indicator_value_Physically_active_adults',
                       'Indicator_value_Obese_adults']]
...
Disease = Master[['Indicator_value_Incidence_of_malignant_melanoma',
                  'Indicator_value_Hospital_stays_for_self-harm',
                  'Indicator_value_Hospital_stays_for_alcohol_related_harm',
                  'Indicator_value_Drug_misuse',
                  'Indicator_value_People_diagnosed_with_diabetes',
                  'Indicator_value_New_cases_of_tuberculosis',
                  'Indicator_value_Acute_sexually_transmitted_infections',
                  'Indicator_value_Hip_fracture_in_65s_and_over']]
print Disease.corr()
...
# are signifcantly skewed. A log transformation will reduce the affects of
# skewness
print ('Display skewness of the Variables')
Disease[['Indicator_value_New_cases_of_tuberculosis',
         'Indicator_value_Acute_sexually_transmitted_infections']].skew()
# Perform the Kolmogorov-Smirnov test for goodness of fit of the Normal
# Distribution
print ('Perform the Kolmogorov-Smirnov test for goodness of fit of the Normal Distribution')
for col in Disease:
...
Life_and_Death = Master[['Indicator_value_Excess_winter_deaths',
                         'Indicator_value_Life_expectancy_male',
                         'Indicator_value_Life_expectancy_female',
                         'Indicator_value_Infant_deaths',
                         'Indicator_value_Smoking_related_deaths',
                         'Indicator_value_Early_deaths_heart_disease_and_stroke',
                         'Indicator_value_Early_deaths_cancer',
                         'Indicator_value_Road_injuries_and_deaths']]
...
# Perform the Kolmogorov-Smirnov test for goodness of fit to the Normal
# Distribution
print ('Perform the Kolmogorov-Smirnov test for goodness of fit of the Normal Distribution')
...
for col in Master_Normalised.columns:
    if col in Log_Transform:
        c = 1.0
        Master_Normalised[col] = Master_Normalised[
            col].apply(lambda x: np.log(c + x))
    else:
        continue
...
Subset_Master_Normailsed = Master_Normalised[
    [
        'Indicator_value_Deprivation',
        'Indicator_value_Drug_misuse',
        'Indicator_value_Acute_sexually_transmitted_infections',
...
Subset_Master_Normailsed_Area = Master_Normalised[
    [
        'Area Name                                           ',
        'Indicator_value_Deprivation',
        'Indicator_value_Drug_misuse',
...
# Plot - Historgrams of Each of the Variables to see there any Outliers in
# the Indicators
...
for i in np.arange(1, Subset_Master_Normailsed_Numerics.shape[1]):
    ...
    plt.scatter(
        Subset_Master_Normailsed_Numerics[
            :, i], Subset_Master_Normailsed_Numerics[
            :, i], c=Subset_Master_Normailsed.icol(
                i + 4), s=60)
...
Subset_Master_Normailsed_Outlier = Subset_Master_Normailsed_Outlier.icol(
    [0, 4, 1, 5, 2, 6, 3, 7, 12])
Subset_Master_Normailsed_Outlier = pd.DataFrame(
    Subset_Master_Normailsed_Outlier)
...
for i in np.arange(1, Subset_Master_Normailsed_Numerics.shape[1]):
    Pearson_Subset = stats.pearsonr(
        Subset_Master_Normailsed_Numerics[
            :, 0], Subset_Master_Normailsed_Numerics[
            :, i])
    print('Pearson Correlation Coefficient between %s and %s: %.4f') %  \
        (Subset_Master_Normailsed.columns[0],
         Subset_Master_Normailsed.columns[i],
         Pearson_Subset[0])

# Plot Relations
for i in np.arange(1, Subset_Master_Normailsed_Numerics.shape[1]):
    plt.figure(Subset_Master_Normailsed.columns[
               i] + ' - Relationship to Dependent')
    plt.scatter(
        Subset_Master_Normailsed_Numerics[
            :, i], Subset_Master_Normailsed_Numerics[
            :, 0], s=60)
...
Subset_Master_Normailsed_1 = Master_Normalised[
    [
        'Indicator_value_Deprivation',
        'Indicator_value_Drug_misuse',
        'Indicator_value_Obese_Children_(Year_6)']]
Subset_Master_Normailsed_1_Columns = Subset_Master_Normailsed_1.columns
Subset_Master_Normailsed_1 = Subset_Master_Normailsed_1.fillna(
    Subset_Master_Normailsed_1.icol(0).mean())
...
print('Model >> %s = %.4f constant + %f%s + %f%s') % \
    (Subset_Master_Normailsed_1_Columns[0],
     model1.params[0],
     model1.params[1],
     Subset_Master_Normailsed_1_Columns[1],
     model1.params[2],
     Subset_Master_Normailsed_1_Columns[2])
...
print('Model >> %s = %.4f constant + %f%s') % \
    (Subset_Master_Normailsed_1_Columns[0], model2.params[
     0], model2.params[1], Subset_Master_Normailsed_1_Columns[1])
...
print(model3.params)
print('Model >> %s = %f%s') % \
    (Subset_Master_Normailsed_1_Columns[0], model3.params[
     0], Subset_Master_Normailsed_1_Columns[1])
...
# Define the Explanatory Variables
Subset_Master_Normailsed_1 = Subset_Master_Normailsed_1[
    (Subset_Master_Normailsed_1[:, 0] > 0)]
x = Subset_Master_Normailsed_1[:, 1]
y = Subset_Master_Normailsed_1[:, 0]
...
print(model4.params)
print('Model >> %s = %f%s') % \
    (Subset_Master_Normailsed_1_Columns[0], model4.params[
     0], Subset_Master_Normailsed_1_Columns[1])
