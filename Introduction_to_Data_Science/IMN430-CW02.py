...
# Import User Data
user_data = pd.read_csv('/home/dan/Desktop/IMN430-CW02/Book_Data/BX-Users.csv'\
                                                                    , sep=';')
# Check Data has been imported correctly
print(user_data.head(5))

# Import Ratings Data
rating_data =  pd.read_csv('/home/dan/Desktop/IMN430-CW02/Book_Data/BX-Book-Ratings.csv' \
                                                                    , sep=';')
# Check Data has been imported correctly
...
# Get a Subset of the Data where No. of Commas = 2 so vectors can be made - [Town, State, Country]
...
user_data['Town'] = user_data.Location.apply(lambda x: x.split(',')[0].lstrip().rstrip())
user_data['State'] = user_data.Location.apply(lambda x: x.split(',')[1].lstrip().rstrip())
user_data['Country'] = user_data.Location.apply(lambda x: x.split(',')[2].lstrip().rstrip())
...
print('Master Dataframe has %i rows and %i columns') % (DF.shape[0], DF.shape[1])
...
country_names = pd.read_csv('/home/dan/Desktop/IMN430-CW02/Book_Data/countries-20140629.csv' \
                                                                    , sep=',')
# Check Data has been irosstmported correctly
print(country_names.head(5))
# Merge Data on to Master Dataframe - Left Join to include only those in the Review Data
...
userRating = DF[['User-ID','Book-Rating']].groupby(by='User-ID', as_index=False)
...
userRating_matrix = userRating.as_matrix(columns=['Standard Deviation', 'Mean'])
...
plt.scatter(userRating_matrix[:,0], userRating_matrix[:,1], c=colors, alpha=0.5)
# No Recognisable Cluster or Pattern
########################################
# Try K-Means to See there are Possible Patterns - Manually Define the Number of Clusters
...
plt.savefig('/home/dan/Desktop/IMN430-CW02/Book_Data/KMeans_Cluster_Reviews.png' \
                                                                    , dpi=120)
plt.show()
########################################
# Create Plot using the data from the Matrix
...
plt.ylabel('Mean')
plt.title('Plot of Reviewers Feedback')
plt.savefig('/home/dan/Desktop/IMN430-CW02/Book_Data/First_plot.png' \
                                                                    , dpi=120)
plt.show()
########################################
...
plt.savefig('/home/dan/Desktop/IMN430-CW02/Book_Data/MeanShift_Cluster_Reviews.png'\
                                                                    , dpi=120)
plt.legend()                                                                    
plt.show()
##############################################################################
...
DF = DF.merge(userRatingDetails, how='left', left_on='User-ID', right_on='User-ID')
...
Label_Merge['Behaviour_Group_Name'] = Label_Merge['Behaviour_Group_Code'].apply(lambda x: getName(x))
...
##############################################################################
# Can a Model be built such that It can determine if a user will be a Always negative User
DF.columns = ['User_ID', 'ISBN', 'Book_Rating', 'Location', 'Age', 'No_Commas', 'Town', 'State', 'Book_Title', 'Book_Author', 'Year_Of_Publication', 'Publisher', 'Image_URL_S', 'Image_URL_M', 'Image_URL_L', 'Correct_Name', 'Latitude', 'Longitude', 'USER_N_Reviews', 'Behaviour_Group_Code', 'ID', 'Reviewer_Mean', 'Reviewer_STD', 'Behaviour_Group_Name', 'lenghtBookTile']
# Create a Boolean Column to ID those in the Always Negative Group
DF['Always_Negative_Boolean'] = DF.Behaviour_Group_Code.apply(lambda x: 1 if x == 2 else 0)
...
scores_acc = cross_val_score(LogisticRegression(), X, y, scoring='accuracy', cv=10)
...
Values = model.predict_proba(np.array([10, 25, 2, 1, 100, int(Test_Country), int(Test_State)]))
print('Prediction of User Being an Always Negative User Class: %.4f %%') % (100*Values[0,1])
...
##############################################################################
###### Does the year of puplication imply popularity? by each group
Year_Popularity = DF[['Behaviour_Group_Name', 'Year_Of_Publication', 'Book_Rating']]
...
Year_Popularity = Year_Popularity.groupby(by=['Behaviour_Group_Name', 'Year_Of_Publication'], \
                                                                as_index=False)
# Obtain the Mean and Standard Deviation for ploting the mean with confidence interval
...
##############################################################################
###### Is the the spread of the distributions of user types even accross each country?
...
Country_Distribution = Country_Distribution['User_ID'].agg({'No. of Reviews by Class': np.size})
# Save dataframe to CSV for Publishing in Tableau
Country_Distribution.to_csv('/home/dan/Desktop/Country_Distribution.csv', sep=',')
...
Publisher_Overview_Num = Publisher_Overview.groupby(by=['Publisher'], as_index=False)
Publisher_Overview_Num = Publisher_Overview_Num['No. of Reviews by Class'].agg({'No. Reviews': np.sum})
# Filter out low values 
Publisher_Overview_Num = Publisher_Overview_Num[(Publisher_Overview_Num['No. Reviews'] > 5)]
