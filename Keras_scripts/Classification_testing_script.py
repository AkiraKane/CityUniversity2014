...
logging.info("Defining Input Parameters")
t0 = time()
directory = "[LOCATION]"
...
def plot_confusion_matrix(
        data, title='Confusion Matrix', cmap=plt.cm.Blues, name=''):
...
for each_name in named_datasets.keys():
    ...
    X_true = np.loadtxt(fname=directory + training_dataset,
                        delimiter=' ')

    logging.info("Getting Prediction for '{}'".format(dataset_name))
    y_pred = model.predict_classes(X_true, batch_size=128, verbose=1)
