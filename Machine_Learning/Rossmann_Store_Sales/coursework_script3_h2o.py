...
# h2o Machine Learning - http://learn.h2o.ai/content/hands-on_training/python_with_h2o.html
...
def split_fit_predict():
    h2o.init(min_mem_size_GB=6)
    X_test = h2o.import_file(path="/".join([os.getcwd(), 'h2o_testset.csv']))
    X = h2o.import_file(path="/".join([os.getcwd(), 'h2o_dataset.csv']))
    # Store the Results in a List
    data = []
    # Cross Validation - Re-Assign a Random Uniform Variable
    for Fold in range(4):
        # Classic Test/Train split
        r = X['Sales'].runif()   # Random UNIForm numbers, one per row
        train = X[ r  < 0.85 ]
        test  = X[ r >= 0.85 ]
        print "Training data has",train.ncol,"columns and",train.nrow,"rows, test has",test.nrow
        # Run DRF
        model = h2o.random_forest(x =train.drop("Sales"),
                                  y=train["Sales"],
                                  validation_x=test.drop("Sales"),
                                  validation_y=test["Sales"],
                                  ntrees=500,
                                  max_depth=50)
        test_mse_drf  = model.model_performance(test ).mse()
        print ", R2 TEST=",test_mse_drf
        
        data.append(test_mse_drf)
