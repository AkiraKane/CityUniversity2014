...
class AcquisitionFunction(object):
    ...
    def full_EI(self, ymax, mean, var, verbose=False):
        ...
        ei[var > 0] = (mean[var > 0] - ymax) * norm.cdf(Z) + \
            var[var > 0] * norm.pdf(Z)
...
class PrintInfo(object):
    ...
    def print_info(self, op_start, i, x_max, ymax, xtrain, ytrain, keys):
        ...
        if self.lvl:
            ...
            print('Iteration: %3i | Last sampled value: %11f' % (
                (i + 1), ytrain[-1]), '| with parameters: ', dict(zip(keys, xtrain[-1])))
            print('               | Current maximum: %14f | with parameters: ' %
                  ymax, dict(zip(keys, xtrain[numpy.argmax(ytrain)])))

            minutes, seconds = divmod(
                (datetime.now() - op_start).total_seconds(), 60)
            print(
                '               | Time taken: %i minutes and %s seconds' %
                (minutes, seconds))
            print('')
    ...
    def print_log(self, op_start, i, x_max, xmins,
                  min_max_ratio, ymax, xtrain, ytrain, keys):
        ...
        if self.lvl:
            ...
            numpy.set_printoptions(precision=4, suppress=True)
            print('Iteration: %3i | Last sampled value: %8f' % (
                (i + 1), ytrain[-1]), '| with parameters: ', dict(zip(keys, return_log(xtrain[-1]))))
            print('               | Current maximum: %11f | with parameters: ' %
                  ymax, dict(zip(keys, return_log(xtrain[numpy.argmax(ytrain)]))))
