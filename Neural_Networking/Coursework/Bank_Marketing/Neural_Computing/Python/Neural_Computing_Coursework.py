...
# Test 4: Accuracy, F1 Score, Log Loss of a boosted Neual Network - Using
...
from sklearn.metrics import f1_score, accuracy_score, log_loss, confusion_matrix, precision_score, recall_score, roc_auc_score, roc_curve
...
def import_Data():
    ...
    Working_DataFrame = pd.read_table(
        '/home/dan/Documents/Dropbox/Data Science Share/City - MSc Data Science/CityUniversity2014/Neural_Networking/Coursework/Version_3_Bank_Marketing/Original_Data/bank-additional-full.csv',
        sep=';')
...
# Get Random stratified split into training set and testing sets to
...
def saving_Data(DF, file_name):
    ...
    DF.to_csv(
        '/home/dan/Documents/Dropbox/Data Science Share/City - MSc Data Science/CityUniversity2014/Neural_Networking/Coursework/Version_3_Bank_Marketing/' +
        file_name,
        sep=',',
        index=False)
    # End Stop Watch
...
def Running_ML_Models(Training_DF, Testing_DF):
    ...
    print("Feature space holds %d observations and %d features" %
    ...
    cm = confusion_matrix(y_Test, Acc2)
    # Save Results
    Results.append(['Support Vector Machines', accuracy(y_Train, Acc1), accuracy(
        y_Test, Acc2), np.float(cm[1, 1]) / (cm[1, 0] + cm[1, 1])])
    ...
    cm = confusion_matrix(y_Test, Acc2)
    # Save Results
    Results.append(['Random Forest', accuracy(y_Train, Acc1), accuracy(
        y_Test, Acc2), np.float(cm[1, 1]) / (cm[1, 0] + cm[1, 1])])
    ...
    cm = confusion_matrix(y_Test, Acc2)
    # Save Results
    Results.append(['K-Nearest-neighbors', accuracy(y_Train, Acc1),
                    accuracy(y_Test, Acc2), cm[1, 1] / (cm[1, 0] + cm[1, 1])])
    ...
    cm = confusion_matrix(y_Test, Acc2)
    # Save Results
    Results.append(['Gradient Boosting Classifier', accuracy(y_Train, Acc1), accuracy(
        y_Test, Acc2), np.float(cm[1, 1]) / (cm[1, 0] + cm[1, 1])])
    ...
    cm = confusion_matrix(y_Test, Acc2)
    # Save Results
    Results.append(['AdaBoost Classifier', accuracy(y_Train, Acc1), accuracy(
        y_Test, Acc2), np.float(cm[1, 1]) / (cm[1, 0] + cm[1, 1])])
    ...
    return pd.DataFrame(Results, columns=[
                        'Model', 'Validation Accuracy', 'Test Accuracy', 'Negative Predictive Value'])
...
def tanh(x):
    ...
    # T.M, Heskes and B. Kappen. Online learning processes in artificial
    # neural networks
...
class NeuralNetwork:
    ...
    def __init__(self, layers, activation='tanh'):
        ...
        "logistic" or "tanh"
        """
        # Switch Function that can change the Activation function used in the
        # Network
    ...
    def fit(self, X, y, learning_rate, epochs):
        ...
        for k in range(epochs):
            ...
            for s in range(1):
                ...
                for l in range(len(self.weights)):
                    ...
                    for l in range(
                            len(a) - 2, 0, -1):  # we need to begin at the second to last layer
                        deltas.append(
                            deltas[-1].dot(self.weights[l].T) * self.activation_deriv(a[l]))
        ...
        # Print a Message
        print(
            "Training Neural Network - Time to Complete: %.4f Seconds" %
            (end - start))
...
def grid_search_nn(DF, Set_Name):
    ...
    Averaged_Results = np.empty(shape=(len(ParameterGrid(param_grid)), 6))
    In = 0
    ...
    for parameters in ParameterGrid(param_grid):
        ...
        for train_index, test_index in kf:
            ...
            nn = NeuralNetwork(
                [X_train.shape[1], parameters['Neurons'], 1], 'tanh')
            # Train the Neural Network
            nn.fit(
                X_train,
                y_train,
            ...
            Results[
                i, ] = [
                accuracy_score(
                    y_val, y_Pred_Val), f1_score(
                    y_val, y_Pred_Val), log_loss(
                    y_val, y_Pred_Val)]
        ...
        Averaged_Results[
            ...
            np.mean(
                ...
                Results[
                    :, 1]), np.mean(
                Results[
                    :, 2]), parameters['Neurons'], parameters['Learning Rate'], parameters['epochs']]
    ...
    # Print a Message
    print(
        "Grid Search Complete - Time to Complete: %.4f Seconds" %
        (end - start))
    # Return Array Back to the Main Function
...
def get_SMOTE(Training_DF):
    ...
    Boosted = np.concatenate(
        (SMOTE(
            Minority, 100, 6), np.ones(
            (len(Minority), 1))), axis=1)
...
def get_RBM_Data(Training_DF):
    ...
    RBM_Data = np.concatenate(
        (r.run_hidden(Synthetic), np.ones(
            (len(Synthetic), 1))), axis=1)
    # Combine Data into a Refined Array
...
def training_mode():
    ...
    test_ind, training_ind = stratified_Sampling(
        Finished_DF['y'], train_allocation=0.1)
    ...
    # Saving Testing and Training Dataframes - Testing Data frame Required for
    # Evaulation Later
...
def make_save_roc_plot(i, Testing_DF_Targets, y_Pred_Val):
    # Get Statistics
    fpr, tpr, thresholds = roc_curve(Testing_DF_Targets, y_Pred_Val)
    roc_auc = roc_auc_score(Testing_DF_Targets, np.round(y_Pred_Val))
    ...
    # Save Figure
    plt.savefig('ROC_Plot_Model_' + str(i) + '.png', dpi=150)


def getConfusionMatrixPlot(true_labels, predicted_labels, i):
    ...
    res = ax.imshow(cm, cmap=plt.cm.binary,
                    interpolation='nearest', vmin=cm.min(), vmax=cm.max())
    # Add a color to the Right hand side
    plt.colorbar(res)
    # Annotate Confustion Matrix
    width = len(cm)
    height = len(cm[0])
    # Add nuerical Labels
    ...
    plt.savefig('Confustion_Model_' + str(i) + '.png', dpi=150)

# Due to the class inbalance the needs to different color plots
...
# Function collects, trains and tests the "Best" Neural Network Models


def testing_mode_results(Collecting_results, i, Results, Training_DF,
                         Training_DF_Targets, Testing_DF, Testing_DF_Targets):
    # Loop through array
    for val in Results.as_matrix():
            # Create Network
        ...
        for row in np.arange(0, Testing_DF_Targets.shape[0]):
            y_Pred_Val[row] = nn.predict(Testing_DF[row])
        # Compute the Confusion Matrix
        cm = confusion_matrix(Testing_DF_Targets, np.round(y_Pred_Val))
        # Collect the Results and Add to Collecting Results Array
        Collecting_results[i, ] = [int(val[3]), np.float(val[4]), int(val[5]),
                                   f1_score(
            Testing_DF_Targets, np.round(y_Pred_Val)),
            accuracy_score(
            Testing_DF_Targets, np.round(y_Pred_Val)),
            log_loss(
            Testing_DF_Targets, np.round(y_Pred_Val)),
        ...
        # Create ROC Plots
        make_save_roc_plot(i, Testing_DF_Targets, y_Pred_Val)
        # Create Confustion Matrix Plots
        getConfusionMatrixPlot(Testing_DF_Targets, np.round(y_Pred_Val), i)
        # Collecting Results iterator
        i += 1
    # Return Results to testing mode function
    return Collecting_results, i
...
def testing_mode():
    ...
    col = Training_DF.columns - ['y']
    Training_DF_Targets = Training_DF['y']
    Training_DF = Training_DF[col]
    SMOTE_Boosted_Targets = SMOTE_Boosted['y']
    SMOTE_Boosted = SMOTE_Boosted[col]
    RBM_Boosted_Targets = RBM_Boosted['y']
    RBM_Boosted = RBM_Boosted[col]
    Testing_DF_Targets = Testing_DF['y']
    Testing_DF = Testing_DF[col].as_matrix()
    # Load in the Results Data
    Unboosted_Results = loading_Data(
        file_name='Grid_Search_Results_Non_Boosted')
    ...
    Unboosted_Results = Unboosted_Results.sort(
        columns=['Avg Accuracy'], ascending=False).head(5)
    SMOTE_Results = SMOTE_Results.sort(
        columns=['Avg Accuracy'],
        ascending=False).head(5)
    RBM_Results = RBM_Results.sort(
        columns=['Avg Accuracy'],
        ascending=False).head(5)
    ...
    Collecting_results = np.zeros(shape=(15, 13))
    i = 0
    # Dataframe
    col_names = [
        'No_Neurons',
    ...
    Collecting_results, i = testing_mode_results(
        Collecting_results, i, Unboosted_Results, Training_DF, Training_DF_Targets, Testing_DF, Testing_DF_Targets)
    # Testing Boosted - SMOTE
    Collecting_results, i = testing_mode_results(
        Collecting_results, i, SMOTE_Results, SMOTE_Boosted, SMOTE_Boosted_Targets, Testing_DF, Testing_DF_Targets)
    # Testing Boosted - RBM
    Collecting_results, i = testing_mode_results(
        Collecting_results, i, RBM_Results, RBM_Boosted, RBM_Boosted_Targets, Testing_DF, Testing_DF_Targets)
    # Save Results to CSV for confirmation
    Collecting_results = pd.DataFrame(Collecting_results, columns=col_names)
    saving_Data(Collecting_results, 'Final_Test.csv')
    # End Stop Watch
    end = time.time()
    # Print a Message
    print(
        'Testing with the Unseen Dataset %.4f Seconds\nIncluding collection of Results\nCreating confusion matrix plots\nROC Curves' %
        (end - start))
    # Return Results
...
if __name__ == "__main__":
    ...
    if testing != 'Yes':
        ...
        Collecting_results = testing_mode()
    # The End!
