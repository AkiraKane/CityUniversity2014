...
from scipy.optimize import minimize                          # Minimisation Algorithm
...
def grid_search_nn(DF, Set_Name):
    ...
    param_grid = {'Neurons': np.arange(75,200,25), 
                  'Learning Rate': np.arange(0.05, 0.15, 0.025),
                  'epochs': np.arange(10000,100000,10000)}
...
def get_RBM_Data(Training_DF):
    ...
    # Train the Machine
    r.train(X_Train, max_epochs = 200)
...
def optimisation_search(model, x0, iterations, bound_parms):
    # Define the Bounds of the Algorithm
    bnds = ((bound_parms[0],None,None), 
            (bound_parms[1],None,None),
            (None,bound_parms[2],None),
            (None,bound_parms[3],None),
            (None,None,bound_parms[4]),
            (None,None,bound_parms[5]))
    # Get the Output of the Algorithm
    if model=='RMB':
        Output = minimize(cross_val_RMB, x0, method='Nelder-Mead', bounds=bnds, options={'disp': True, 'maxfev': iterations})
    elif model=='SMOTE':
        Output = minimize(cross_val_SMOTE, x0, method='Nelder-Mead', bounds=bnds, options={'disp': True, 'maxfev': iterations})
    else:
        Output = minimize(cross_val_basic, x0, method='Nelder-Mead', bounds=bnds, options={'disp': True, 'maxfev': iterations})
    # Return the Output to the Main Function
    return Output
    
def cross_val_RMB(x0):
    print (x0)
    # Dataframe to use:
    DF = RBM_Boosted
    # Construct the K-Fold
    ...
    col = DF.columns - ['y']
    X_Train = DF[col].as_matrix()
    # Save Results into a Array
    Results = np.empty(shape = (10, 1)); i = 0
    ...
    for train_index, test_index in kf:
            ...
            y_train, y_val = y_Train[train_index], y_Train[test_index]
            # Create the Neural Network
            nn = NeuralNetwork([X_train.shape[1], int(x0[1]), 1], 'tanh')
            # Train the Neural Network
            nn.fit(X_train, y_train, x0[0], int(x0[2]))
            # Prediction Using the Validation Set
            y_Pred_Val = np.zeros(shape=(X_val.shape[0], 1))
            # Validation
            for row in np.arange(0, X_val.shape[0]):
                y_Pred_Val[row] = np.round(nn.predict(X_val[row]))
            # Save Accuracy into Array
            Results[i,] = [accuracy_score(y_val, y_Pred_Val)]
            # Increment Row Operator by One
            i += 1
    # Return the Output to Dataframe
    return 1-np.mean(Results[:,0])
    
def cross_val_SMOTE(x0):
    # Dataframe to use:
    DF = SMOTE_Boosted
    # Construct the K-Fold
    kf = KFold(len(DF), n_folds=10, shuffle=True)
...
if __name__ == "__main__":
    ...
    if Getting_Models=='Yes':
        ...
        print('Evaluation of the Best NN Models')
        
        # Import Data - Training Data
        Training_DF = pd.read_csv('/home/dan/Documents/Dropbox/Data Science Share/City - MSc Data Science/CityUniversity2014/Neural_Networking/Coursework/Version_3_Bank_Marketing/Training.csv', sep=',')
        # Import Data - Testing Data        
        Testing_DF = pd.read_csv('/home/dan/Documents/Dropbox/Data Science Share/City - MSc Data Science/CityUniversity2014/Neural_Networking/Coursework/Version_3_Bank_Marketing/Testing.csv', sep=',')
        # Import SMOTE Data
        SMOTE_Boosted = pd.read_csv('/home/dan/Documents/Dropbox/Data Science Share/City - MSc Data Science/CityUniversity2014/Neural_Networking/Coursework/Version_3_Bank_Marketing/SMOTE_Boosted.csv', sep=',')
        # Import RBM Data
        RBM_Boosted = pd.read_csv('/home/dan/Documents/Dropbox/Data Science Share/City - MSc Data Science/CityUniversity2014/Neural_Networking/Coursework/Version_3_Bank_Marketing/RBM_Boosted.csv', sep=',')
        
        # Maximum Number of Iterations to Evaluate
        iterations = 20        
        
        # RBM Initialisation Values taken from Output of Previous Section
        x0      = [0.075, 75, 90000] # LR, Neurons, Epochs
        
        # Run the Optimisation Algorithm
        Output_RBM = optimisation_search('RBM', x0, iterations, [0.95*x0[0], 1.05*x0[0] , 0.95*x0[1], 1.05*x0[1], x0[2], x0[2]])
        
        # SMOTE Initialisation Values taken from Output of Previous Section
        x0      = [0.075, 75, 90000] # LR, Neurons, Epochs        
        
        # Run the Optimisation Algorithm
        Output_SMOTE = optimisation_search('SMOTE', x0, iterations, [0.95*x0[0], 1.05*x0[0] , 0.95*x0[1], 1.05*x0[1], x0[2], x0[2]])
        
        # Basic Initialisation Values taken from Output of Previous Section
        x0      = [0.075, 75, 90000] # LR, Neurons, Epochs        
        
        # Run the Optimisation Algorithm
        Output_basic = optimisation_search('basic', x0, iterations, [0.95*x0[0], 1.05*x0[0] , 0.95*x0[1], 1.05*x0[1], x0[2], x0[2]])
