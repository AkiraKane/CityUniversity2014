function [Error] = Cross_Validation( Parameters )
    ...
    configuration.hidNum = [500, 30];      % Number of Hidden Nodes by layer
    configuration.activationFnc = {'tansig','tansig','tansig'}; % Activation Functions
    % http://uk.mathworks.com/help/nnet/ug/multilayer-neural-network-architecture.html
    configuration.eNum   = 4000;                                  % Max Number of Epochs to Train On
    configuration.bNum   = 1;                                   % Batch Size
    %configuration.params = [Parameters(3)/100 0.1 0.1 0.0001];      % Additional parameters
    configuration.params = [0.01 0.0 0.0 0.0000];      % Additional parameters
...
%         end;
    ...
    for i = 1:k;                                                % Loop through each fold
        ...
        Validation_Set = X(Indices == i, 1:size(X,2));        % Create the Validation Data
        Validation_Set_Labels = t(Indices == i,1);      % Create the Validation Labels
        Training_Set = X(Indices ~=i, 1:size(X,2));           % Create the Training Data
        Training_Set_Labels = t(Indices ~=i, 1);        % Create the Training Labels
