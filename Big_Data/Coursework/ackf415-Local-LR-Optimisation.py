...
def logisticRegression(trainingRDD, trainingRDDHashed, testRDDHashed, iterations, minibatch, stepsize):
    # Train a Naive Bayes Model
    trainedModel = LogisticRegressionWithSGD.train(trainingRDD, iterations=iterations, miniBatchFraction=minibatch, regType="l2", intercept=True, regParam=0.1, step=stepsize)
    # Test on Validation and Test Sets
...
if __name__ == "__main__":
            ...
            for i in np.arange(0,1):
                ...
                for k in np.arange(1,2):
                    ...
                    for j in np.arange(0,1):
                        ...
                        batches = [0.3,0.25,0.225,0.2,0.175,0.15,0.1,0.05,0.025,0.01]
                        for l in np.arange(0,10):
                            ...
                            for p in np.arange(0,4):
                                ...
                                try:                                
                                    timeIt = time()
                                    AccuracyV, fScoreV, AccuracyT, fScoreT = logisticRegression(trainingRDD, trainingRDDHashed, testRDDHashed, iterations, minibatch, stepsize)
                                    resultList.append([hashsize, i+1, k, 3, AccuracyV, fScoreV, AccuracyT, fScoreT, time() - timeIt, iterations, minibatch, stepsize])
    ...
    df = pd.DataFrame(resultList, columns=['Hashsize', 'Subject', 'Cross Validation Fold', 'Model', 'Training Accuracy', 'Training F-Score', 'Test Accuracy', 'Test F-Score', 'Time to Evaluate Model','Number of Iterations','Minibatch Size','Step Size'])
    df.to_csv('/home/dan/Desktop/IMN432-CW01/Results' + str(hashsize) + '.csv', sep=',')  
