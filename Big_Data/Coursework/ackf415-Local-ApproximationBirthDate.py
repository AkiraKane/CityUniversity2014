...
def processFile(x, i, fileEbook, stop_words):
    ...
    text = sc.textFile(x) \
        .zipWithIndex() \
        .map(lambda x_y4: (x_y4[0].lower(), x_y4[1]))
    # Find Number of Lines in Text
    text_len = text.count()
    # Find the Last Line of Header
    headerLine = text.filter(lambda x_y5: findHeader(x_y5[0]) == True) \
        .map(lambda x_y6: x_y6[1]) \
        .collect()
    ...
    footerLine = text.filter(lambda x_y7: (findFooter(x_y7[0])) == True) \
        .map(lambda x_y8: x_y8[1]) \
        .collect()
    ...
    Ebook = text.filter(lambda x_y9: ('[ebook' in x_y9[0]) | ('[etext' in x_y9[0]) | ('[eook' in x_y9[0])) \
                .map(lambda x_y10: findEBookNo(x_y10[0])) \
                .collect()
    # Extract the File name from Root
    filename = FileExtract(x)
    # Subset of Text
    text =  text.filter(lambda x_y11: x_y11[1] > headerLine) \
                .filter(lambda x_y12: x_y12[1] < footerLine)
    # Extract the list of Word Frequency pairs per file (as an RDD)
    text =  text.flatMap(lambda x_y13: (re.split('\W+', x_y13[0]))) \
    ...
    text.map(lambda x: (filename, [x[0], x[1], float(MaxFreq)])) \
        .map(lambda x_y15: (x_y15[0], [x_y15[1][0], float(x_y15[1][1] / x_y15[1][2])])) \
        .map(lambda x_y16: (x_y16[0], [[x_y16[1][0], x_y16[1][1]]])) \
        .repartition(8) \
        .reduceByKey(add) \
...
def xmlExtractDate(files, table):
    if '.rdf' in files:
        ...
        try:
            ...
            creatorDetails = ebookChild.find(
                '{http://purl.org/dc/terms/}creator')
            creatorDetailsAgent = creatorDetails.find(
                '{http://www.gutenberg.org/2009/pgterms/}agent')
            Date = creatorDetailsAgent.find(
...
if __name__ == "__main__":
    ...
    if len(getDirectory(directory[9])) == 1:
        ...
        # Check the Other way to exclude books in Text Part where the Date was
        # not found
