    ...
    for root, dirs, files in os.walk(directory):
        ...
        for file in files:
            ...
            if (os.path.getsize(f) < 1048576) & (
                    '-' not in FileExtract(f)) & (f != directory):
...
def findEBookNo(x):
    # If Statement to Identify the 3 Classification I have found in the Main
    # Dataset
    if 'ebook' in x:
        return ebookNo(x)
    elif 'etext' in x:
...
def processFile(x, i, fileEbook, stop_words):
    ...
    text = sc.textFile(x) \
        .zipWithIndex() \
        .map(lambda x_y5: (x_y5[0].lower(), x_y5[1]))
    # Find Number of Lines in Text
    text_len = text.count()
    # Find the Last Line of Header
    headerLine = text.filter(lambda x_y6: findHeader(x_y6[0]) == True) \
        .map(lambda x_y7: x_y7[1]) \
        .collect()
    ...
    footerLine = text.filter(lambda x_y8: (findFooter(x_y8[0])) == True) \
        .map(lambda x_y9: x_y9[1]) \
        .collect()
    ...
    Ebook = text.filter(lambda x_y10: ('[ebook' in x_y10[0]) | ('[etext' in x_y10[0]) | ('[eook' in x_y10[0])) \
                .map(lambda x_y11: findEBookNo(x_y11[0])) \
                .collect()
    # Extract the File name from Root
    filename = FileExtract(x)
    # Subset of Text
    text =  text.filter(lambda x_y12: x_y12[1] > headerLine) \
                .filter(lambda x_y13: x_y13[1] < footerLine)
    # Extract the list of Word Frequency pairs per file (as an RDD)
    text =  text.flatMap(lambda x_y14: (re.split('\W+', x_y14[0]))) \
    ...
    text.map(lambda x: (filename, [x[0], x[1], float(MaxFreq)])) \
        .map(lambda x_y28: (x_y28[0], [x_y28[1][0], float(x_y28[1][1] / x_y28[1][2])])) \
        .map(lambda x_y29: (x_y29[0], [[x_y29[1][0], x_y29[1][1]]])) \
        .repartition(1) \
        .reduceByKey(add) \
        .saveAsPickleFile('/data/student/ackf415/TF/Word_Freq/' + str(filename))
    # Stop Clock
    fin = time()
...
def xmlExtract(x, table):
    ...
    if '.rdf' in x:
        ...
        for subject in allSubjects:
            # Find the Values
            x = subject.find(
                '*').find('{http://www.w3.org/1999/02/22-rdf-syntax-ns#}value')
            # Add to a list
...
def processXML(table):
    ...
    tf_idf_directory = getDirectory('/data/student/ackf415/TF/TF_IDF')
    ...
    ebookNumbers = ebookNumbers.map(lambda x_y16: x_y16[0]) \
        .collect()
    ...
    df = pd.DataFrame(
        ...
        columns=[
            'Subject',
            'Number of Files with Subject'])
    df.to_csv('IMN432-CW1/Top_10_Subjects_TF.csv', sep=',')
    # Save Locatio of top 10 Subjects
    savePath = '/data/student/ackf415/TF/processXML/'
...
def makeSets(RDD, trainingList, testList, subject_lists, hashsize):
    ...
    trainingRDDHashed = RDD.filter(lambda x_y17: x_y17[0] in trainingList) \
        .map(lambda x_y18: (checkFile(x_y18[0], subject_lists), x_y18[1])) \
        .map(lambda f_wl: (hashVector(f_wl[0], f_wl[1], hashsize)))
    trainingRDD = trainingRDDHashed.map(
        lambda f_x: LabeledPoint(f_x[0], f_x[1]))
    # Create the validation Set - Correctly at the data as labelled Points
    testRDDHashed = RDD.filter(lambda x_y19: x_y19[0] in trainingList) \
        .map(lambda x_y20: (checkFile(x_y20[0], subject_lists), x_y20[1])) \
        .map(lambda f_wl21: (hashVector(f_wl21[0], f_wl21[1], hashsize)))
    testRDD = testRDDHashed.map(lambda f_x22: LabeledPoint(f_x22[0], f_x22[1]))
...
def getAccuracy(resDict, count):
    ...
    truePos = resDict[(1, 1,)]
    falsePos = resDict[(0, 1,)]
    trueNeg = resDict[(0, 0,)]
    falseNeg = resDict[(1, 0,)]
...
if __name__ == "__main__":
    ...
    directory = [
        '/data/extra/gutenberg/text-full',
        '/data/student/ackf415/TF/TF_IDF/',
        '/data/student/ackf415/TF/Word_Freq/',
        '/data/student/ackf415/TF/IDF/',
        '/data/student/ackf415/TF/IDF/IDF-Pairs',
        '/data/student/ackf415/TF/IDF',
        '/data/student/ackf415/TF/TF_IDF/TF_IDF_File',
        '/data/student/ackf415/TF/processXML/',
        '/data/extra/gutenberg/TF/meta/',
        '/data/student/ackf415/TF/TF_IDF',
        '/data/student/ackf415/TF/processXML/Subject',
    ...
    config = SparkConf().setMaster("local[6]")
    config.set("spark.executor.memory", "25")
    ...
    if len(pickleWordF) < 1:
        ...
        stop_words = sc.textFile(
            directory[11]).flatMap(
            lambda x: re.split(
                ',',
                x)).collect()
        # Loop Through Each of the Files to Extract the Required Parts
        for i in np.arange(
                0, N):  # Change to specify the number of Files to Process
    ...
    if len(getDirectory(directory[1])) < 1:
        ...
        IDF = sc.union([sc.pickleFile(i) for i in allFolders])
        # Rearrange RDD into correct the correct format
        TF_IDF = IDF.map(lambda x_y_z: (x_y_z[0], [[pair[0], x_y_z[1] * ast.literal_eval(pair[1])] for pair in x_y_z[2]])) \
                    .flatMap(lambda x_y2: [(pairs[0], [[x_y2[0], str(pairs[1])]]) for pairs in x_y2[1]]) \
                    .reduceByKey(add) \
                    .map(lambda x_y3: (x_y3[0], [[pairs[0], ast.literal_eval(pairs[1])] for pairs in x_y3[1]])) \
                    .repartition(2)
    ...
    if len(getDirectory(directory[9])) == 1:
        # Get a list of the directory where the TF.IDF Vector is stored as a
        # Pickle
        ...
        # Get Ebook numbers for analysis and also remove Null Values
        ebookNumbers = tf_idf.map(lambda x_y4: x_y4[0]) \
                             .filter(lambda x: len(x) > 1) \
                             .collect()
        # Results List - [Hashsize, Subject, CrossVal, Model, Training Acc,
        # Training F1, Test Acc, Test F1, Time]
        resultList = []
        # Define the Hashsize
        hashsizeVec = [10000]
        for hashsize in hashsizeVec:
            ...
            for i in np.arange(0, 10):
                ...
                trainingRDD, testRDD, trainingRDDHashed, testRDDHashed = makeSets(
                    modelSet, trainingList, testList, subject_lists, hashsize)
                # Naive Bayes ML Algorithm
                timeIt = time()
                AccuracyV, fScoreV, AccuracyT, fScoreT = naiveBayes(
                    trainingRDD, trainingRDDHashed, testRDDHashed)
                resultList.append([hashsize,
                                   i + 1,
                                   1,
                                   1,
                                   AccuracyV,
                ...
                timeIt = time()
                AccuracyV, fScoreV, AccuracyT, fScoreT = decisionTree(
                    trainingRDD, trainingRDDHashed, testRDDHashed, testRDD)
                resultList.append([hashsize,
                                   i + 1,
                                   1,
                                   2,
                ...
                timeIt = time()
                AccuracyV, fScoreV, AccuracyT, fScoreT = logisticRegression(
                    trainingRDD, trainingRDDHashed, testRDDHashed)
                resultList.append([hashsize,
                                   i + 1,
                                   1,
    ...
    if len(fileEbook) > 0:
        print('Average Time to Process Files:  %.3f Seconds') % (
            np.mean(np.array([i[4] for i in fileEbook])))
    ...
    print('Total Run Time:                 %.3f Seconds') % (
        time() - start_time_overall)
    print('################################################')
    ######################################################
    ...
    df.to_csv('IMN432-CW1/Results-Full' + str(hashsize) + '.csv', sep=',')
