...
def table2():
    ...
    another_rdd = sc.parallelize([("John", "England", 120), ("Jenny", "Spain", 45), ("Sarah", "Japan", 55)])
    ...
    # Save Data Above as a Parqet File
    # SchemaRDDs can be saved as Parquet files, maintaining the schema information.
    another_schemardd.saveAsParquetFile("/home/dan/Desktop/People_Details.parquet")
    # Register this SchemaRDD as a table.
...
if __name__ == "__main__":
    ...
    if os.path.isdir('/home/dan/Desktop/People_Age.parquet')==False:
        ...
        parquetFile = sqlCtx.parquetFile("/home/dan/Desktop/People_Age.parquet")
    ...
    if os.path.isdir('/home/dan/Desktop/People_Details.parquet')==False:
        ...
        print('Reading in Pre-made Parquet File')
        # Read in the Parquet file created above.        
        parquetFile = sqlCtx.parquetFile("/home/dan/Desktop/People_Details.parquet")
        # Parquet files can also be registered as tables and then used in SQL statements.
        parquetFile.registerTempTable("People_Details")

    #####################################################
    # Extract First SQL Statement in Spark SQL
    teenagers = sqlCtx.sql("SELECT Person_First_Name, age FROM People_Age WHERE age <= 20").collect()
    # Print First Extract
    ...
    query2 = sqlCtx.sql("SELECT Person_First_Name FROM People_Details").collect()
    # For Loop to Iterate throuch Query line by line
    for names in query2:
        # Print Statement
    ...
    for row in inner_join:
        # Print Statement
        print('Name = %s, Country = %s, Avg Spend = %i, Age = %i') % (row[0], row[1], row[2], row[3])
    # Left Join
    print('\nLeft Join Example\n')
